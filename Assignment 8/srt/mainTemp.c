...
void* run(void* pdata){
    struct PixelRange* data = (struct PixelRange*) pdata;

    for(int px = data->startX; px < data->endX; px++){
        const double x = data->pixel_dx * ((double)( px-(width/2) ));
        for(int py = 0; py < height; py++){
            const double y = data->pixel_dy * ((double)( py-(height/2) ));
            Vec3 pixel_color;
            ...
            for( int xs=-halfSamples; xs<=halfSamples; ++xs )
            {
                ...
                {
                    double subx = x + ((double)xs)*data->subsample_dx;
                    double suby = y + ((double)ys)*data->subsample_dy;

                    /* construct the ray coming out of the camera, through
                     * the screen at (subx,suby)
                     */
                    ray_t pixel_ray;
                    copy( pixel_ray.org, data->camera_pos );
                    Vec3 pixel_target;
                    set( pixel_target, subx, suby, z );
                    sub( pixel_ray.dir, pixel_target, data->camera_pos );
                    norm( pixel_ray.dir, pixel_ray.dir );

                    Vec3 sample_color;
                    copy( sample_color, data->bg_color );
                    /* trace the ray from the camera that
                     * passes through this pixel */
                    trace( data->scene, sample_color, &pixel_ray, 0 );
            ...
            for( int i=0; i<3; i++){
                ...
                data->arr[px][py][i] = scaled_color[i];
            }
...
main( int argc, char **argv )
{
    ...
    /* for every pixel */
    //we need to refactor the code so it is 2d array-ized and we can print them after.
    
    float *** arr = (float ***) malloc(width * sizeof(float **));
    for(int i = 0; i < width; i++){
        arr[i] = (float **) malloc(height * sizeof(float *));
        for(int j = 0; j < height; j++){
            arr[i][j] = (float *) malloc(3 * sizeof(float));
        }
    }

    struct PixelRange * pxlRanges = (struct PixelRange *) malloc(nthreads * sizeof(struct PixelRange));
    
    pthread_t tids[nthreads];
    int curThread = 0;
    for(curThread = 0; curThread < nthreads; curThread++){
        struct PixelRange * pxlRange = (struct PixelRange *) malloc(sizeof(struct PixelRange));
        pxlRange->startX = curThread*width;
        pxlRange->endX = curThread*(width+1)-1;
        pxlRange->pixel_dy = pixel_dy;
        pxlRange->pixel_dx = pixel_dx;
        pxlRange->subsample_dx = subsample_dx;
        pxlRange->subsample_dy = subsample_dy;
        pxlRange->scene = &scene;
        for(int i = 0; i < 3; i++){
            pxlRange->bg_color[i] = bg_color[i];
            pxlRange->camera_pos[i] = camera_pos[i];
        }
        pxlRange->arr = arr;
        int ret = pthread_create(&tids[curThread], NULL, run, (void *)pxlRange); 
        if(ret){
            printf("ERROR. Could not create p_thread");
        }
    }
    ...
    for( int px=0; px<width; ++px )
    {
        for( int py=0; py<height; ++py )
        {
             ...
             */
            printf( "%.0f %.0f %.0f\n",
		    arr[px][py][0], arr[px][py][1], arr[px][py][2] );
        }
        printf( "\n" );
    }
    free(arr);
