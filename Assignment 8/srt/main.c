...
void * run(void * obj){
    ...
    /* for every pixel */
    for( int px=everyNth; px<width; px+=nthreads )
    {
...
main( int argc, char **argv )
{
    ...
    for(int i = 0; i < nthreads; i++){
        gaps[i] = i;
        int ret = pthread_create(&tids[i], NULL, run, (void *)(gaps+i));
        if(ret){
            fprintf(stderr, "Was not able to create new thread.");
        }
    }
